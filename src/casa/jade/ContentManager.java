 ...
 */
@Deprecated //TODO tagged for deletion -- not used
public class ContentManager implements Serializable {
   ...
   * @param ontoName
   * @param content
   * @return either a byte[] or a String, depending on the type of the Codec specified by <em>codecName</em>.
   * @throws CodecException
   * @throws OntologyException
   * @author Rob Kremer
   */
	public Object makeExpressionString(String codecName, String ontoName, ContentElement content) throws CodecException, OntologyException {
		Codec    codec = lookupLanguage(codecName);
		if (codec == null) {
			throw new CodecException("Unknown language "+codecName);
		}
		Ontology o = null;
		if (ontoName != null) {
			o = lookupOntology(ontoName);
			if (o == null) {
				throw new OntologyException("Unknown ontology "+ontoName);
			}
		}
		Ontology onto  = getMergedOntology(codec, o);
	 ...
	 * to the ontology used for this operation.
	 */
	public AbsContentElement extractAbsContent(MLMessage msg) throws CodecException, OntologyException {
		return getAbs(msg.getParameter(ML.LANGUAGE), msg.getParameter(ML.ONTOLOGY), msg.getParameter(ML.CONTENT));
	}
	 ...
	 * to the ontology used for this operation.
	 */
	public AbsContentElement getAbs(String codecName, String ontoName, String exp) throws CodecException, OntologyException {
		Codec    codec = lookupLanguage(codecName);
		if (codec == null) {
			throw new CodecException("Unknown language "+codecName);
		}
		...
		AbsContentElement content = decode(exp, codec, onto);
	...
	private Object encode(AbsContentElement content, Codec codec, Ontology onto) throws CodecException, OntologyException { 
		if (codec instanceof ByteArrayCodec)
			return ((ByteArrayCodec) codec).encode(onto, content);
		else if (codec instanceof StringCodec)
			return ((StringCodec) codec).encode(onto, content);
		else
			throw new CodecException("UnsupportedTypeOfCodec");
	}
	...
	private AbsContentElement decode(Object buf, Codec codec, Ontology onto) throws CodecException, OntologyException { 
		try {
			if (codec instanceof ByteArrayCodec)
				return ((ByteArrayCodec) codec).decode(onto, (byte[])buf);
			else if (codec instanceof StringCodec)
				return ((StringCodec) codec).decode(onto, (String)buf);
			else
				throw new CodecException("UnsupportedTypeOfCodec");
		} catch (Throwable e) {
